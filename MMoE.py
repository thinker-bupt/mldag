...
class MMoE(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.gate_layers = [tf.keras.layers.Dense(self.expert_nums, activation='softmax') for _ in range(self.task_nums)]
...
class AITM(object):
    ...
    def _init_graph_AITM(self):
        ...
        with self.graph.as_default():
            ...
            mmoe_result = MMoE(hidden_units=64, expert_nums=4, task_nums=2)(feature_embedding)
            # ctr
            self.tower_click = mmoe_result[0]
            ...
            # cvr
            self.tower_purchase = mmoe_result[1]
...
#                 self.conv - self.click,
#                 tf.zeros_like(
#                     self.click))
#             self.loss = self.loss + args.weight * \
#                 tf.reduce_mean(label_constraint, axis=0)

#             self.train_labels_click * (tf.maximum(self.conv - self.click, tf.zeros_like(self.click))
            
            # 点击cvr强制大于未点击cvr
#             nctr = tf.reduce_sum((1.0 - self.train_labels_click) * self.conv) / tf.reduce_sum(1.0 - self.train_labels_click)
#             ct_cvr = tf.reduce_sum(self.train_labels_click * self.conv) / tf.reduce_sum(self.train_labels_click)
#             click_cvr_pc = nctr - ct_cvr
#             self.click_cvr_pc = tf.maximum(click_cvr_pc, tf.zeros([1])) * args.weight
            
#             # 转化cvr强制大于点击
#             ncv_ctr = tf.reduce_sum((1.0 - self.train_labels_purchase) * self.train_labels_click * self.conv) / tf.reduce_sum((1.0 - self.train_labels_purchase) * self.train_labels_click)
#             cvr = tf.cond(tf.reduce_sum(self.train_labels_purchase) > 0., lambda: tf.reduce_sum(self.train_labels_purchase * self.conv) / tf.reduce_sum(self.train_labels_purchase), lambda: tf.ones([1]))
#             conv_cvr_pc = ncv_ctr - cvr
#             self.conv_cvr_pc = tf.maximum(conv_cvr_pc, tf.zeros([1])) * args.weight
             
#             self.loss = self.loss + self.click_cvr_pc + self.conv_cvr_pc
...
if __name__ == '__main__':
    ...
    with open(file_name, 'a+') as f:
        f.writelines('model,pctr_auc,pcvr_auc,pctcvr_auc\n')
    ...
    for i in range(6):
        ...
        with open(file_name, 'a+') as f:
            f.writelines('MMoE_epoch%d,%.6f,%.6f,%.6f\n' % tuple([i] + test_result))
